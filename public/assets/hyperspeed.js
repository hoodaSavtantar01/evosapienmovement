...
(function () {
  ...
  class App {
    constructor(container, options = {}) {
      ...
      this.disposed = false;
      this.visible = true; // gated by an IntersectionObserver in init()
    ...
    init() {
      ...
      // Pause the (expensive) postprocessing render whenever the hero scrolls
      // out of view so it doesn't compete with the page for GPU/main-thread
      // while the user is scrolling other sections.
    ...
    tick() {
      ...
      if (this.hasValidSize) {
        // Always advance the clock (keeps delta small so there's no jump on
        // resume), but only render/update while on-screen and the tab is active.
